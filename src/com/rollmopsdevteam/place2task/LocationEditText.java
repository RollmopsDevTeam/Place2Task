...
public class LocationEditText extends AutoCompleteTextView {
	...
	private void init(Context context) {
		_locationAdapter = new ArrayAdapter<String>(context, android.R.layout.simple_dropdown_item_1line);
	...
	private class LocationSearchTask extends AsyncTask<String, Void, List<Address> > {
        ...
        protected List<Address> doInBackground(String... locationName) {
            ...
            try {
                // Getting a maximum of _maxNumberLines Address that matches the input text
                addresses = new Geocoder(getContext(), Locale.getDefault()).getFromLocationName(locationName[0], _maxNumberLines);
            } catch (IOException e) {
			 ...
			 if(addresses != null && addresses.size() > 0) {
				 ...
				 for( Address address : addresses ) {
					 ...
					 for( int line = 0; line < address.getMaxAddressLineIndex();line++) {
						 addressString += line > 0 ? ", " + address.getAddressLine(line) : address.getAddressLine(line);
					 }
